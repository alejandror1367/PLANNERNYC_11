...
import * as exportPdf from './exportPdf.js';
...
function initHeaderActions() {
  ...
  if (exportPdfBtn) {
    on(exportPdfBtn, 'click', () => exportPdf.exportItinerary());
  }
...
function init() {
  ...
  // Tema (claro/oscuro) — lo primero para evitar parpadeo
  theme.init();

  const versionEl = $('#app-version');
  if (versionEl) versionEl.textContent = `${APP_VERSION} · live`;

  // 0. Pestaña inicial según fase del viaje:
  //    - Durante el viaje → "Hoy" (dashboard copiloto)
  //    - Antes/después    → "Resumen"
  //    Solo si el usuario NO llegó con un hash explícito en la URL.
  if (!window.location.hash) {
    const phase = getTripPhase().phase;
    window.location.hash = phase === 'during' ? '#today' : '#overview';
  }
