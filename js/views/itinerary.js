...
let hasAnimatedIn = false;  // animar entrada solo la primera vez (Fase 8.1)
let forecast = null;        // pronóstico en vivo (Fase 9.3), null hasta cargar
 ...
 */
function renderActivity(act, city) {
  ...
  // Botón "Cómo llegar" (Fase 9.1) — solo si la actividad es mapeable
  let mapBtn = '';
  if (maps.isMappable(act)) {
    const url = maps.directionsUrl(act, city);
    mapBtn = `
      <a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer"
         class="activity-btn activity-btn--map"
         aria-label="Cómo llegar a ${escapeHtml(act.name || 'este lugar')}"
         title="Cómo llegar">📍</a>`;
  }
    ...
    <div class="activity ${stateCls}" data-activity-id="${escapeHtml(act.id || '')}">
      <div class="activity__row">
        <div class="activity__main">
          ...
          ${act.location ? `<p class="activity__location">📍 ${escapeHtml(act.location)}</p>` : ''}
...
/**
 * Genera el mini-mapa del día (Fase 9.1B). Solo si hay 2+ paradas
 * mapeables (con una sola no tiene sentido un "mapa de ruta").
 */
function renderDayMap(day) {
  const mappable = (day.activities || []).filter((a) => maps.isMappable(a));
  if (mappable.length < 2) return '';

  const svg = maps.renderDayMapSvg(day.activities, day.city);
  const routeUrl = maps.dayRouteUrl(day.activities, day.city);
  const names = maps.mappableNames(day.activities);

  // Leyenda: lista numerada de paradas
  const legend = names.map((nm, i) =>
    `<li class="day-map__legend-item"><span class="day-map__legend-num">${i + 1}</span> ${escapeHtml(nm)}</li>`
  ).join('');

  return `
    <div class="day-map">
      <div class="day-map__head">
        <span class="day-map__kicker">★ Ruta del día ★</span>
        <span class="day-map__count">${mappable.length} paradas</span>
      </div>
      ${svg}
      <ol class="day-map__legend">${legend}</ol>
      ${routeUrl ? `
        <a href="${escapeHtml(routeUrl)}" target="_blank" rel="noopener noreferrer"
           class="btn btn--accent btn--full day-map__route-btn">
...
/**
 * Genera el weather-strip del día. Usa el clima EN VIVO si ya se
 * cargó el forecast; si no, cae al clima del seed (Fase 9.3).
 */
function renderWeatherStrip(day) {
  const energyPart = day.energy ? `· ENERGÍA: ${escapeHtml(day.energy)}` : '';

  // Clima en vivo si está disponible
  if (forecast) {
    const iso = weather.dayNumberToISO(day.day);
    const w = weather.dayWeather(forecast, iso, day.city);
    if (w) {
      const temps = (w.max !== null && w.min !== null) ? `${w.max}° / ${w.min}°` : '';
      const rain = (w.precip !== null && w.precip >= 30) ? ` · 💧 ${w.precip}%` : '';
      return `
        <div class="weather-strip weather-strip--live">
          <span>${w.icon}</span> <b>${escapeHtml(w.label)}</b>
          ${temps ? `<span class="weather-strip__temps">${temps}</span>` : ''}
          ${rain}
          ${energyPart}
        </div>
      `;
  ...
  // Fallback: clima del seed
  if (day.weather || day.energy) {
    return `
...
function renderDay(day, animate) {
  ...
  const activities = (day.activities || []).map((a) => renderActivity(a, day.city)).join('');
  const motionCls = animate ? ' motion-item' : '';
  return `
    <article class="day-card${motionCls}${isOpen ? ' is-open' : ''}" data-day="${escapeHtml(String(day.day))}">
...
function render(state) {
  ...
  const animate = !hasAnimatedIn;
  hasAnimatedIn = true;
  const html = state.itinerary.map((d) => renderDay(d, animate)).join('');
...
export function mount() {
  ...
  // Cargar el clima en vivo (Fase 9.3) en segundo plano. Cuando
  // llega, re-renderizamos para mostrar los strips actualizados.
  weather.getForecast().then((data) => {
    if (data) {
      forecast = data;
      if (mounted) render(store.getState());
    }
  }).catch(() => { /* sin clima, la app sigue igual */ });
