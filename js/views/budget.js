...
import { computeInsights } from '../budget.js';
...
/**
 * Card de insights automáticos (Fase 8.6).
 * Devuelve string vacío si no hay suficientes gastos (<5).
 */
function tplInsights(expenses, rate) {
  const tp = getTripPhase(new Date(), (store.getState().itinerary || []).length);
  const insights = computeInsights(expenses, rate, tp);
  if (!insights) return '';

  const lines = [];

  // Categoría top
  if (insights.topCategory && insights.topCategory.pct >= 0) {
    lines.push(
      `Lo que más pesa es <b>${escapeHtml(insights.topCategory.name)}</b>, ` +
      `con ${fmtUSD(insights.topCategory.totalUSD)} (${insights.topCategory.pct}% del total).`
    );
  }

  // Día más caro
  if (insights.priciestDay) {
    const dateLabel = formatDayLabel(insights.priciestDay.date);
    lines.push(
      `El día de mayor gasto fue <b>${escapeHtml(dateLabel)}</b> con ${fmtUSD(insights.priciestDay.totalUSD)}.`
    );
  }

  // Promedio diario
  if (insights.avgPerActiveDay > 0) {
    lines.push(
      `Promedio de <b>${fmtUSD(insights.avgPerActiveDay)}</b> por día con gastos.`
    );
  }

  // Proyección (solo durante el viaje)
  if (insights.projection) {
    lines.push(
      `Si mantienen este ritmo, el viaje cerraría cerca de <b>${fmtUSD(insights.projection.total)}</b> ` +
      `(proyectado desde ${insights.projection.basis} ${insights.projection.basis === 1 ? 'día' : 'días'}).`
    );
  }

  // Alerta de ratio personal
  let warning = '';
  if (insights.personalRatioWarning) {
    warning = `
      <p class="insights-card__warning">
        ⚠ Casi todo está marcado como <b>personal</b>. Si compartieron algún gasto,
        recuerden marcarlo como "shared" para que el balance sea justo.
      </p>
    `;
  }
  ...
  return `
    <article class="card insights-card">
      <header class="insights-card__head">
        <p class="insights-card__kicker">★ Insights ★</p>
        <h3 class="insights-card__title">Lectura rápida de gastos</h3>
      </header>
      <ul class="insights-list">
        ${lines.map((l) => `<li class="insights-list__item">${l}</li>`).join('')}
      </ul>
      ${warning}
    </article>
  `;
...
/**
 * Formatea 'YYYY-MM-DD' a algo legible tipo "mar 7 oct".
 */
...
function tplPaymentsHistory(settlements, rate) {
  ...
  const items = settlements.slice().reverse().map((s) => {
    ...
    return `
      <li class="payment-item ${stateCls}">
        ...
        <div class="payment-item__main">
          <p class="payment-item__title">${escapeHtml(fromName)} pagó a ${escapeHtml(toName)}${stateBadge}</p>
...
function tplExpenseItem(e, rate) {
  ...
  const stateCls = [
    e._pending ? 'is-pending' : '',
    e._queued  ? 'is-queued'  : '',
    e._failed  ? 'is-failed'  : '',
  ].filter(Boolean).join(' ');
  const itemClass = ('expense-item ' + stateCls).trim();

  // Badge de estado para items encolados o fallidos
  let stateBadge = '';
  if (e._failed) {
    stateBadge = '<span class="state-badge state-badge--failed" title="No se pudo sincronizar">⚠ falló</span>';
  } else if (e._queued) {
    ...
    stateBadge = '<span class="state-badge state-badge--pending" title="Guardando...">guardando…</span>';
  }
  ...
  return `
    <li class="${itemClass}">
      ...
      <div class="expense-item__main">
        <p class="expense-item__desc">${escapeHtml(e.desc)}${stateBadge}</p>
...
/**
 * Skeleton premium del budget: imita la estructura real.
 * Kicker → Overview dark → Analytics 2-col → Settlement 3-col → Form.
 */
function skeletonTemplate() {
  return `
    <div class="budget-kicker">
      <div class="skeleton skeleton--text skeleton--short"></div>
      <div class="skeleton skeleton--text-xl skeleton--narrow"></div>
    </div>

    <article class="card card--dark budget-overview">
      <div class="skeleton skeleton--text skeleton--dark skeleton--short" style="margin-bottom: var(--space-4);"></div>
      <div class="skeleton-grid-2">
        <div>
          <div class="skeleton skeleton--text skeleton--dark skeleton--narrow"></div>
          <div class="skeleton skeleton--text-xl skeleton--dark skeleton--medium"></div>
          <div class="skeleton skeleton--text skeleton--dark skeleton--short"></div>
        </div>
        <div>
          <div class="skeleton skeleton--text skeleton--dark skeleton--narrow"></div>
          <div class="skeleton skeleton--text-xl skeleton--dark skeleton--medium"></div>
      ...
      <div class="skeleton skeleton--text skeleton--dark skeleton--short" style="margin: var(--space-4) 0 var(--space-3);"></div>
      <div class="skeleton-grid-2">
        <div class="skeleton skeleton--block skeleton--dark" style="margin: 0;"></div>
        <div class="skeleton skeleton--block skeleton--dark" style="margin: 0;"></div>
      </div>
    </article>
    ...
    <div class="analytics-grid" style="margin-top: var(--space-6);">
      ...
      <article class="card analytics-card">
        <div class="skeleton skeleton--text-lg skeleton--medium"></div>
        <div class="skeleton skeleton--tall" style="margin: var(--space-3) auto; max-width: 200px;"></div>
      </article>
    </div>
...
async function handleAddExpense(e) {
  ...
  try {
    const result = await store.addExpense({ desc, amount, currency, payer, type, category, city });
    if (result && result._queued) {
      toast.warning('Sin conexión — guardado localmente, se sincronizará al volver');
    } else {
...
async function handleMarkPaid(btn) {
  ...
  try {
    const result = await store.addSettlement({
      from: from,
      to: to,
      amountUSD: amount,
    ...
    if (result && result._queued) {
      toast.warning('Sin conexión — pago guardado localmente');
    } else {
...
function setupDelegation() {
  ...
  delegate(panelEl, 'click', '[data-action="delete-expense"]', async function(e, btn) {
    ...
    // Animación de salida antes de borrar (Fase 8.2)
    const li = btn.closest('.expense-item');
    if (li) li.classList.add('motion-out');

    try {
      await store.deleteExpense(id);
      toast.success('Gasto eliminado');
    } catch (err) {
      if (li) li.classList.remove('motion-out');
  ...
  delegate(panelEl, 'click', '[data-action="delete-settlement"]', async function(e, btn) {
    ...
    const li = btn.closest('.payment-item');
    if (li) li.classList.add('motion-out');
