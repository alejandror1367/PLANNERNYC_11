...
const initialState = {
  ...
  settlements:   [],
  schemaVersion: null,  // null hasta primera carga
...
/**
 * Conecta el estado de la cola con el estado del store.
 * Cuando un item está 'failed', marca el item optimista correspondiente
 * con _failed: true (y limpia _queued).
 * Cuando vuelve a 'pending', vuelve a _queued: true.
 *
 * Esto se llama una sola vez al arrancar la app.
 */
let queueSubscriptionInitialized = false;
export function initQueueIntegration() {
  if (queueSubscriptionInitialized) return;
  queueSubscriptionInitialized = true;

  queue.subscribe((items) => {
    // Crear un mapa clientId → status
    const statusByClientId = {};
    for (const it of items) {
      const cid = it.data && it.data.clientId;
      if (cid) statusByClientId[cid] = it.status;
    }
    ...
    const newExpenses = state.expenses.map((e) => {
      if (!e._clientId) return e;
      const qStatus = statusByClientId[e._clientId];
      if (qStatus === 'failed' && !e._failed) {
        dirty = true;
        return { ...e, _failed: true, _queued: false };
      }
      if ((qStatus === 'pending' || qStatus === 'retrying') && !e._queued) {
        dirty = true;
        return { ...e, _queued: true, _failed: false };
      }
      // Si ya no está en la cola, limpiar flags (porque se sincronizó)
      if (!qStatus && (e._queued || e._failed)) {
        dirty = true;
        const { _queued, _failed, ...rest } = e;
        return rest;
      }
      return e;
    });

    // Actualizar settlements (mismo patrón)
    const newSettlements = state.settlements.map((s) => {
      if (!s._clientId) return s;
      const qStatus = statusByClientId[s._clientId];
      if (qStatus === 'failed' && !s._failed) {
        dirty = true;
        return { ...s, _failed: true, _queued: false };
      }
      if ((qStatus === 'pending' || qStatus === 'retrying') && !s._queued) {
        dirty = true;
        return { ...s, _queued: true, _failed: false };
      }
      if (!qStatus && (s._queued || s._failed)) {
        dirty = true;
        const { _queued, _failed, ...rest } = s;
        return rest;
      }
      return s;
    });

    // Actualizar itinerario (anidado: días → actividades)
    let itineraryDirty = false;
    const newItinerary = state.itinerary.map((day) => {
      const newActivities = (day.activities || []).map((a) => {
        if (!a._clientId) return a;
        const qStatus = statusByClientId[a._clientId];
        if (qStatus === 'failed' && !a._failed) {
          itineraryDirty = true;
          return { ...a, _failed: true, _queued: false };
        }
        if ((qStatus === 'pending' || qStatus === 'retrying') && !a._queued) {
          itineraryDirty = true;
          return { ...a, _queued: true, _failed: false };
        }
        if (!qStatus && (a._queued || a._failed)) {
          itineraryDirty = true;
          const { _queued, _failed, ...rest } = a;
          return rest;
        }
        return a;
      });
      return newActivities === day.activities ? day : { ...day, activities: newActivities };
    });

    if (dirty || itineraryDirty) {
      const partial = {};
      if (dirty) {
        partial.expenses = newExpenses;
        partial.settlements = newSettlements;
      }
      if (itineraryDirty) partial.itinerary = newItinerary;
      setState(partial);
    }
  });
}
...
// PHOTODUMP (Fase 8.9)
// ============================================================
// Los links del photodump se guardan como JSON serializado en la
// misma hoja Wishlist (celda A2), reutilizando updateWishlist como
// transporte. El estado expone una lista estructurada parseada.
//
// Formato: [{ id, title, url, desc }]

/**
 * Parsea el contenido de wishlist a una lista de links.
 * Tolerante: si el contenido no es JSON válido (ej. texto viejo),
 * devuelve lista vacía sin romper.
 */
export function getPhotodump() {
  const raw = state.wishlist || '';
  if (!raw.trim()) return [];
  try {
...
/**
 * Persiste una lista de links del photodump (reemplaza toda la lista).
 * Optimistic + encolable offline.
 * @param {Array<{id,title,url,desc}>} links
 */
export async function savePhotodump(links) {
  const serialized = JSON.stringify(links || []);
  const prev = state.wishlist;

  setState({ wishlist: serialized });

  try {
    await api.updateWishlist(serialized);
    persistCache();
  } catch (err) {
    if (isNetworkError(err)) {
      queue.enqueue('updateWishlist', { wishlist: serialized });
      persistCache();
      return;
    }
    setState({ wishlist: prev });
...
/**
 * Agrega un link al photodump.
 * @param {{title, url, desc}} link
 */
export async function addPhotodumpLink(link) {
  const list = getPhotodump();
  const newLink = {
    id: 'pd_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6),
    title: link.title || '',
    url: link.url || '',
    desc: link.desc || '',
  };
  return savePhotodump([...list, newLink]);
}

/**
 * Elimina un link del photodump por id.
 */
export async function removePhotodumpLink(id) {
  const list = getPhotodump().filter((l) => l.id !== id);
  return savePhotodump(list);
}
...
/**
 * Actualiza la metadata de un día (título + energía). Optimistic. Encolable.
 * @param {Object} data - { day, title, energy }
 */
export async function updateDay(data) {
  const dayNum = Number(data.day);

  // Guardar valores previos para revertir
  const prevDay = state.itinerary.find((d) => Number(d.day) === dayNum);
  const previous = prevDay
    ? { title: prevDay.title, energy: prevDay.energy }
    : { title: '', energy: '' };

  // Optimistic: aplicar title/energy al día
  mutateItinerary((itinerary) =>
    itinerary.map((d) =>
      Number(d.day) === dayNum
        ? { ...d, title: data.title, energy: data.energy }
        : d
    )
  );

  try {
    await api.updateDay({ day: dayNum, title: data.title, energy: data.energy });
    persistCache();
  } catch (err) {
    if (isNetworkError(err)) {
      queue.enqueue('updateDay', { day: dayNum, title: data.title, energy: data.energy });
      persistCache();
      return;
    }
    // Revertir
    mutateItinerary((itinerary) =>
      itinerary.map((d) =>
        Number(d.day) === dayNum
          ? { ...d, title: previous.title, energy: previous.energy }
          : d
      )
    );
    throw err;
