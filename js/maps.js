...
export function dayRouteUrl(activities, city) {
  ...
  // Cada parada se codifica individualmente. Los waypoints se unen
  // con '|' (separador que la API espera). encodeURIComponent ya
  // codifica cada parada; el '|' separador va como %7C UNA sola vez.
  const origin = encodeURIComponent(buildQuery(mappable[0], city));
  const destination = encodeURIComponent(buildQuery(mappable[mappable.length - 1], city));

  // Waypoints = paradas intermedias (sin origen ni destino)
  const middle = mappable.slice(1, -1);
  const waypoints = middle
    .map((a) => encodeURIComponent(buildQuery(a, city)))
    .join('%7C'); // %7C = | (separador entre waypoints)
