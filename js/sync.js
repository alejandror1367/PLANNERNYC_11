...
import { UI, SCHEMA_VERSION } from './config.js';
...
let schemaMismatchNotified = false;  // evita múltiples banners por el mismo mismatch
...
function bindStoreToSyncStatus() {
  store.subscribe((state) => {
    // Detección de schema mismatch (Fase 6.7)
    if (state.schemaVersion && state.schemaVersion !== SCHEMA_VERSION && !schemaMismatchNotified) {
      schemaMismatchNotified = true;
      errorBanner.showSchemaMismatch({
        backendVersion: state.schemaVersion,
        clientVersion: SCHEMA_VERSION,
        onReload: () => {
          // Limpiar cache para forzar carga fresca
          try { localStorage.removeItem('tp2.cache'); } catch (e) {}
